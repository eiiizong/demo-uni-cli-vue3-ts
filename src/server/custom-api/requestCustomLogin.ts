...
/**
 * 自定义登录 业务逻辑请自行更改
 * @param {Boolean} isShowNotLoggedInPrompt [true] 用户未登录时 是否进行提示
 * @param {Boolean} isShowLoading 是否显示加载中动画 默认值 true
 */
const requestCustomLogin = (isShowNotLoggedInPrompt: boolean = true, isShowLoading = true): Promise<any> => {
  return new Promise((resolve, reject) => {
    login()
      .then((res) => {
        const { code } = res
        if (code) {
          requestGetOpenId(code, isShowLoading)
            .then((res) => {
